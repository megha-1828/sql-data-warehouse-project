# 🥈 Silver Layer - Data Transformation (MySQL) 
## 📌 Overview This script transforms data from Bronze → Silver layer.
### Operations: 
                * Data cleaning 
                * Standardization 
                * Deduplication 
...
SELECT
    ...
    CASE 
        WHEN TRIM(sls_order_dt) = '' OR sls_order_dt = 0 OR LENGTH(TRIM(sls_order_dt)) != 8
            THEN NULL
        ELSE STR_TO_DATE(TRIM(sls_order_dt), '%Y%m%d')
    END,

    CASE 
        WHEN TRIM(sls_ship_dt) = '' OR sls_ship_dt = 0 OR LENGTH(TRIM(sls_ship_dt)) != 8
            THEN NULL
        ELSE STR_TO_DATE(TRIM(sls_ship_dt), '%Y%m%d')
    END,

    CASE 
        WHEN TRIM(sls_due_dt) = '' OR sls_due_dt = 0 OR LENGTH(TRIM(sls_due_dt)) != 8
            THEN NULL
